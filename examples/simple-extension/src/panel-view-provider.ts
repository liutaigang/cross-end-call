import { ExtensionContext, Uri, Webview, WebviewPanel, WebviewView, WebviewViewProvider } from 'vscode';
...
export class PanelViewProvider implements WebviewViewProvider {
  ...
  protected async getWebviewHtml(webview: Webview) {
    const webviewUri = webview.asWebviewUri(Uri.joinPath(this.context.extensionUri, PanelViewProvider.WEBVIEW_DIST_DIR)).toString();
    ...
    const htmlPath = join(this.context.extensionPath, PanelViewProvider.WEBVIEW_INDEX_PATH);
    ...
    return await modifyHtml(htmlText, {
      ...
      oncomment(data) {
        const hasMark = data?.toString().toLowerCase().includes(PanelViewProvider.WEBVIEW_INJECT_IN_MARK);
